        ...
        while(!arb.isTimeUp() && maxDepth <= board.numEmptyCells()) {
    		...
    		for(int i = 0; i < 7; i++) {//Cycle through each column
    			if (board.isValidMove(i)) {//If the move in this column is valid, then
	    			...
	    			int score = minimax(board, maxDepth,false, arb);//Calculate the score by running the minimax
...
//    			return the heuristic value of node 
    	...
    	if(isMaximizing) {
    		...
    		for(int i = 0; i < 7; i++) {//Cycle through each column
    			if (board.isValidMove(i)) {//If the move in this column is valid, then
	    			board.move(i, id);//Make the move to this column
	    			bestScore = Math.max(bestScore, minimax(board, depth - 1, false, arb));//Calculate the highest score
	    			board.unmove(i, id);//Reverse the move
    			}
    		}
    		...
    		for(int i = 0; i < 7; i++) {//Cycle through each column
    			if (board.isValidMove(i)) {//If the move in this column is valid, then
	    			...
	    			bestScore = Math.min(bestScore, minimax(board, depth - 1, true, arb));//Calculate the minimum of the move
